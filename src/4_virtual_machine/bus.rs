...
use crate::virtual_machine::devices::{uart::Uart, clint::Clint, plic::Plic};
...
impl SystemBus {
    ...
    fn route(&mut self, addr: u64) -> Option<(&mut dyn MemoryAccess, u64)> {
        match addr {
            ...
            a if a >= CLINT_BASE && a <= CLINT_END =>
                Some((&mut self.clint, addr - CLINT_BASE)),
            a if a >= PLIC_BASE && a <= PLIC_END =>
                Some((&mut self.plic, addr - PLIC_BASE)),
            _ => {
    ...
    pub fn ram_size(&self) -> usize { self.ram.size() as usize }
    pub fn rom_size(&self) -> usize { self.rom.size() as usize }

    // Direct access to devices for interrupt handling
    pub fn uart_mut(&mut self) -> &mut Uart { &mut self.uart }
    pub fn clint_mut(&mut self) -> &mut Clint { &mut self.clint }
    pub fn plic_mut(&mut self) -> &mut Plic { &mut self.plic }
}
