...
use crate::virtual_machine::cpu::alu::{
    self, RM_DYN,
};
...
pub enum ExecResult {
    ...
    Jump          { next_pc: u64 },
    Load          { rd: usize, addr: u64, funct3: u8, next_pc: u64 },
    Store         { addr: u64, val: u64, funct3: u8, next_pc: u64 },
    FLoad         { rd: usize, addr: u64, funct3: u8, next_pc: u64 },
    FStore        { addr: u64, bits: u64, funct3: u8, next_pc: u64 },
    Atomic        { funct5: u8, aq: bool, rl: bool, funct3: u8, rd: usize, addr: u64, val: u64, next_pc: u64 },
...
pub fn execute(
    ...
    match insn {
        ...
        DecodedInsn::Branch { funct3, rs1, rs2, imm } => {
        ...
        DecodedInsn::Load { funct3, rd, rs1, imm } => {
        ...
        DecodedInsn::Store { funct3, rs1, rs2, imm } => {
            let addr = regs.read_x(*rs1).wrapping_add(*imm as u64);
        ...
        DecodedInsn::AluImm { funct3, funct7, rd, rs1, imm } => {
            exec_alu_imm(*funct3, *funct7, *rd, *rs1, *imm, regs, pc)
        }

        DecodedInsn::AluImm32 { funct3, funct7, rd, rs1, imm } => {
            exec_alu_imm32(*funct3, *funct7, *rd, *rs1, *imm, regs, pc)
        }

        DecodedInsn::Alu { funct3, funct7, rd, rs1, rs2 } => {
            exec_alu(*funct3, *funct7, *rd, *rs1, *rs2, regs, pc)
        }

        DecodedInsn::Alu32 { funct3, funct7, rd, rs1, rs2 } => {
            exec_alu32(*funct3, *funct7, *rd, *rs1, *rs2, regs, pc)
        }

        DecodedInsn::Fence { .. } => Ok(ExecResult::Fence { next_pc: pc.wrapping_add(4) }),

        DecodedInsn::FenceI => Ok(ExecResult::FenceI { next_pc: pc.wrapping_add(4) }),
        ...
        DecodedInsn::FLoad { funct3, rd, rs1, imm } => {
            let addr = regs.read_x(*rs1).wrapping_add(*imm as u64);
            Ok(ExecResult::FLoad {
                rd: *rd,
                addr,
        ...
        DecodedInsn::FStore { funct3, rs1, rs2, imm } => {
            let addr = regs.read_x(*rs1).wrapping_add(*imm as u64);
        ...
        DecodedInsn::FOp { funct5, fmt, rm, rd, rs1, rs2 } => {
            exec_fp_op(*funct5, *fmt, *rm, *rd, *rs1, *rs2, regs, csrs, pc)
        }

        DecodedInsn::FMac { op, fmt, rm, rd, rs1, rs2, rs3 } => {
            exec_fmac(op, *fmt, *rm, *rd, *rs1, *rs2, *rs3, regs, csrs, pc)
        }

        DecodedInsn::Atomic { funct5, aq, rl, funct3, rd, rs1, rs2 } => {
...
fn exec_alu_imm(
    ...
    Ok(ExecResult::WriteInt { rd, val: result, next_pc: pc.wrapping_add(4) })
}
...
fn exec_fp_op(
    ...
    let rm = if rm == RM_DYN { csrs.rounding_mode() } else { rm };
    ...
    match funct5 {
        ...
        0b00000 => {
            ...
            Ok(ExecResult::WriteFpFlags { rd, bits, fflags, next_pc })
        }
        // FSUB
        0b00001 => {
            let (bits, fflags) = match fmt {
        ...
        0b10100 => {
            ...
            Ok(ExecResult::WriteIntFlags { rd, val, fflags, next_pc })
        }
            ...
            let (bits, fflags) = match fmt {
                ...
                1 => {
                    ...
                    let fflags: u8 = match rs2 {
                        2 => {
                            if (result as i64) != (src as i64) { 0x01 } else { 0 }
                        }
                        3 => {
                            if (result as u64) != src { 0x01 } else { 0 }
                        }
                        _ => 0,
                    };
        ...
        0b01001 => {
            ...
            Ok(ExecResult::WriteFpFlags { rd, bits: val_d.to_bits(), fflags: 0, next_pc })
        }
