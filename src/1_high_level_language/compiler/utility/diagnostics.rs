use crate::high_level_language::token::Span;
...
impl Diagnostics {
    ...
    pub fn warn_at(&mut self, span: Span, message: impl Into<String>) {
        self.entries.push(
            Diagnostic::new(DiagnosticLevel::Warning, message).with_span(span),
        );
    }
    ...
    pub fn error_at(&mut self, span: Span, message: impl Into<String>) {
        self.entries.push(
            Diagnostic::new(DiagnosticLevel::Error, message).with_span(span),
        );
    }
