...
pub fn highlight_assembly(theme: &egui::Style, code: &str) -> LayoutJob {
    ...
    let instructions: &[&str] = &[
        ...
        // A‑extension
        "lr.w", "sc.w", "amoadd.w", "amoswap.w", "amoxor.w", "amoand.w", "amoor.w",
        "amomin.w", "amomax.w", "amominu.w", "amomaxu.w",
        "lr.d", "sc.d", "amoadd.d", "amoswap.d", "amoxor.d", "amoand.d", "amoor.d",
        ...
        "bgt", "ble", "bgtu", "bleu",
        "j", "jr", "ret", "call", "tail",
        "fmv.s", "fmv.d", "fneg.s", "fneg.d", "fabs.s", "fabs.d",
    ];
    ...
    let directives: &[&str] = &[
        ...
        ".byte", ".half", ".word", ".dword", ".float", ".double",
        ".asciz", ".ascii", ".string",
        ".skip", ".space", ".zero",
        ".equ", ".set", ".type", ".size",
        ".section",
    ...
    for segment in code.split_inclusive('\n') {
        ...
        while start < len {
            ...
            if bytes[start].is_ascii_alphabetic() || bytes[start] == b'_' || bytes[start] == b'.' {
                while end < len && (bytes[end].is_ascii_alphanumeric() || bytes[end] == b'_' || bytes[end] == b'.') {
                    end += 1;
                }
                let word = &line[start..end];
                ...
                if end < len && bytes[end] == b':' {
                    ...
                    job.append(
                        ":",
                        0.0,
                        egui::TextFormat { font_id: font_id.clone(), color: theme.visuals.text_color(), ..Default::default() },
                    );
                    start = end + 1;
                    ...
                    start = end;
                } else if int_regs.contains(&word) || fp_regs.contains(&word) || word.starts_with('x') || word.starts_with('f') {
            ...
            if bytes[start].is_ascii_digit() || (bytes[start] == b'0' && start+1 < len && (bytes[start+1] == b'x' || bytes[start+1] == b'b')) {
